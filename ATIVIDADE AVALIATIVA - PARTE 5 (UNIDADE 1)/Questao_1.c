...
// Atualmente vetorEntidades não segue nenhuma ordem específica.
// Tarefa: implemente uma função que reordene vetorEntidades para que a entidade mais próxima do jogador fique sempre em vetorEntidades[1] (a posição 0 é sempre o jogador).
// Dicas:
// • A ordenação deve trocar apenas os ponteiros guardados no vetor (Entidade *tmp = vetorEntidades[i]; ...), nunca o conteúdo das structs.
// • Calcule a distância usando jogador->pos e vetorEntidades[i]->pos.
// • Chame essa função uma vez por quadro, antes do laço de desenho.
